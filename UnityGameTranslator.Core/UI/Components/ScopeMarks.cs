...
namespace UnityGameTranslator.Core.UI.Components
{
    ...
    public static class ScopeMarks
    {
        ...
        public static void Adorn(GameObject buttonObj, EditSide side)
        {
            ...
            // 🔴 **The room the marks take is now counted by ButtonLabelFitter, not added here.**
            //
            // This used to raise the caller's minWidth by exactly what it inserts, which was right
            // as far as it went — and only as far as it went. The fitter rewrites minWidth and
            // preferredWidth on every label change, from the label's own width, so the allowance
            // survived only until the first translated or recomputed label. A long one then made
            // the fitter size the button to the TEXT alone, the marks pushed that text out, and
            // "Review Branches (2)" ran across its neighbour.
            //
            // The fitter measures whatever shares the row with the label, so a caller no longer
            // has anything to compute — which also means the next control laid inside a button is
