...
namespace UnityGameTranslator.Core
{
    ...
    public static class CustomFontLoader
    {
        ...
        public class MsdfAtlasData
        {
            ...
            // Version of the generating pipeline (TtfFontPipeline.PipelineVersion).
            // Absent in old .gen.json files (deserializes to 0) → those caches are
            // re-rasterized when the source TTF is available. User-provided
            // msdf-atlas-gen JSONs never carry it and are not concerned (no TTF).
        ...
        private static bool TryLoadGenCache(CustomFontInfo fontInfo, string cacheDir, string fontName)
        {
            ...
            try
            {
                ...
                // Pipeline version check: atlases generated by an older pipeline carry
                // known rendering defects (1px inter-glyph spacing → underlay bleed,
                // fixed 48px sampling → jagged edges at large sizes). Re-rasterize when
                // the source TTF is available; without a TTF the old cache stays usable.
                if (atlasData.pipelineVersion < Rasterizer.TtfFontPipeline.PipelineVersion &&
