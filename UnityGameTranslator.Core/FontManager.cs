...
namespace UnityGameTranslator.Core
{
    ...
    public static class FontManager
    {
        ...
        /// replacement — a universal signal that the game itself drives this component's
        /// rendering (typewriter reveal, per-frame animation…). Static components (menus,
        /// labels) keep our font and never trip this. Callers use it to STOP generating
        /// their own mesh events on such components and let the game own the reveal
        /// (issue #21). Not game-specific: any game that re-asserts fonts trips it.
        /// </summary>
        ...
        public static Font GetUnityReplacementFont(string originalFontName)
        {
            ...
            if (!_unityFallbackFonts.TryGetValue(originalFontName, out var replacementFont))
            {
                ...
                {
                    // Try to find the original game font by name
                    _gameUnityFonts.TryGetValue(originalFontName, out originalGameFont);

                    // Fallback: search all loaded fonts
                    if (originalGameFont == null)
                    {
                        var allFonts = TypeHelper.FindAllObjectsOfType(typeof(Font));
                        foreach (var f in allFonts)
                        {
