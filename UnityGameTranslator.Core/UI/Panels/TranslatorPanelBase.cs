...
namespace UnityGameTranslator.Core.UI.Panels
{
    ...
    public abstract partial class TranslatorPanelBase : PanelBase, IScreen
    {
        ...
        /// as hosts by <see cref="Layout"/>.
        /// </summary>
        private void BuildScrollableLayout(int cardWidth, out Host scrollContent, out Host buttonRow)
        {
            ...
            UIStyles.CreateScrollablePanelLayout(ContentRoot, out GameObject scroll, out GameObject footer, cardWidth);

            if (HasFlexibleContent)
            {
                ConfigureScrollContentForFlex(scroll);
            }
        ...
        /// <summary>
        /// The contextual help bar, pinned between the scroll area and the footer buttons. The
        /// zone translates its own label as it writes it (the code owns that Text), so there is
        /// nothing to register here.
        /// </summary>
        private Components.HelpZone BuildHelpZone(Host buttonRow, string defaultText)
        {
            var footer = buttonRow?.Object;
            var helpZone = new Components.HelpZone();
            var parent = footer != null ? footer.transform.parent.gameObject : ContentRoot;
            helpZone.CreateUI(parent, defaultText);

            // Pin the bar just above the footer buttons
            if (footer != null && helpZone.Root != null)
                helpZone.Root.transform.SetSiblingIndex(footer.transform.GetSiblingIndex());

            return helpZone;
        }

        /// <summary>
        /// A fixed (non-scrolling) header container pinned between the title bar and the scroll
        /// area — the top mirror of the HelpZone/footer pattern. As a direct child of ContentRoot
        /// it is automatically counted as chrome by MeasureChromeHeight.
        /// </summary>
        private Host BuildFixedHeader(string name)
        {
        ...
        /// <summary>
        /// The window's width as laid out, or the declared one before any layout has run — what the
        /// title's scope strip reasons from when its own row cannot answer yet.
        /// </summary>
        private float WindowWidth()
        {
            return Rect != null && Rect.rect.width > 1f ? Rect.rect.width : PanelWidth;
        }
